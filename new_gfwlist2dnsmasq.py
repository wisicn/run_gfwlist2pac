...
def parse_args():
    ...
    parser.add_argument('-d', '--dns', dest='dnsconf', required=True,
                        help='the DNS configure of the gfwlist hostname, for example,\
                        "127.0.0.1#5353"', metavar='DNS')
...
def generate_dnsmasq(domains, dnsconf, ipsetname):
    # render the pac file
    dnsmasq_content=''
    for domain in domains:
        dnsmasq_content+='server=/'+domain+'/'+dnsconf+'\n'
        dnsmasq_content+='ipset=/'+domain+'/'+ipsetname+'\n'
...
def main():
    ...
    dns_content = generate_dnsmasq(domains, args.dnsconf,'gfwlist')
